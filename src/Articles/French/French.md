...
- **Place of articulation**:
    ...
    - **Dental**: tip of tongue + front teeth. These aren't used in French but contribute considerably to the distinctive sounding of **English**. Specifically, //θ// and //ð//, as in ^__th__ink^/^ba__th__^, and ^__th__is^ and ^mo__th__er^. These are good examples for you can still end up with accented pronounciation with live curses. If one's mother language (like Hungarian or Italian) only has the sounds //s// and //d// (as in "soap" and "domain") then you can easily mistake them for //θ// and //ð// because they sound very similar and yet the former two are ^alveolar^, while the latter two are ^dental^.  
    ...
    - **Postalveolar (start of steep slope/start of curvature of hard palate)**: tongue tip + postalveolar/prepalatal region. This area is that just behind the ^alveolar ridge^ where the rooftop begins to steeply curve upwards, where the hard palate/rooftop begins (hence **pre**-palatal). Examples are //ʃ// in "shoe" or "ship", and //ʒ// in ^__g__enre^, ^plea__s__ure^ or ^trea__s__ure^.
